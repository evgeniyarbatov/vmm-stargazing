...
`docs/index.html` is the contents page. Nights, stops, sky charts, and constellation plates are linked from there and from each other. PNGs live in `docs/plots/`. Raw data is `docs/data.json`. Best stargazing places are `docs/stargazing-spots.gpx` (waypoints). Each chart has a download link.
